...
query_feat_vec = scalar_df.sample().drop(['Unnamed: 0', 'filename', 'category'], axis='columns')

# get rest of feature vectors without the one selected above
rest_of_feat_vecs = scalar_df.drop(scalar_df.index[query_feat_vec.index]).drop(['Unnamed: 0', 'filename', 'category'], axis='columns')
...
distances = []
for i in range(len(rest_of_feat_vecs)):

    # check that index is not the one we dropped
    if i != query_feat_vec.index:
        target_vec = rest_of_feat_vecs.loc[i]
        dist = euclidean_distance(query_feat_vec, target_vec)
